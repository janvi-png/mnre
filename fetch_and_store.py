...
def ingest_csv_to_sqlite(csv_path):
    ...
    for idx, row in df.iterrows():
        dname = None
        for k in ['district_name','district','districtname']:
            if k in row and pd.notna(row[k]):
                dname = str(row[k]).strip(); break
        if not dname:
            continue
        dcode = None
        for k in ['district_code','district_code ']:
            if k in row and pd.notna(row[k]):
                dcode = str(row[k]).strip(); break
        ...
        for k in ['year','financial_year','fy']:
            if k in row and pd.notna(row[k]):
                try: year = int(float(row[k])); break
                except: pass
        for k in ['month','mnth']:
            if k in row and pd.notna(row[k]):
                try: month = int(float(row[k])); break
                except: pass
        def getnum(*cands):
            for c in cands:
                if c in row and pd.notna(row[c]):
                    try: return float(row[c])
                        ...
                        try: s = str(row[c]).replace(',',''); return float(s)
                        except: pass
            return None
    ...
    print(f"Ingested rows (attempted inserts): {inserted}"); return OUT_DB

if __name__ == '__main__':
    csv = fetch_csv(); db = ingest_csv_to_sqlite(csv); print('Done. DB at:', db)
